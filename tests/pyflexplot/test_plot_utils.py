...
def assert_summary_dict_element_is_subelement(obj_sub, obj_super):
    ...
    if obj_sub == obj_super:
        ...
        raise AssertionError(
            f"elements differ: {obj_sub} != {obj_super}")
...
class TestAxesTextBox_Serialize:
    """Serialize all relevant information about an ``TextBoxAxes``."""

    def test_TODO(self):

        fig, ax_ref = plt.subplots()
        rect_lbwh = [1, 1, 3, 2]
        box = TextBoxAxes(fig, ax_ref, rect_lbwh)
        ...
        #ipython(globals(), locals(), f"{type(self).__name__}")
