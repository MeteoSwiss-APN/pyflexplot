...
def ensemble_probability(arr: np.ndarray, thr: float = 0.0, thr_type: str = "lower") -> np.ndarray:
    ...
    if thr_type == "lower":
        ...
        raise ValueError(f"invalid threshold type '{thr_type}' (neither 'lower' nor 'upper')")
    n_mem = arr.shape[0]
...
class Cloud:
    ...
    def arrival_time(self) -> np.ndarray:
        ...
        arr[:] = np.where(m_will_appear, m_will_appear[::-1].cumsum(axis=0)[::-1] * self.ts, arr)

        # Points where first cloud has appeared before the current time step
        m_has_appeared = ~m_clear_since_start & ~m_will_appear & ~m_clear_till_end & ~self.mask[:1]
...
def merge_fields(flds: Sequence[np.ndarray], op: Union[Callable, Sequence[Callable]] = np.nansum) -> np.ndarray:
